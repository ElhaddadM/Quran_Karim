    ...
    <div className="App-header cov " style={{ backgroundColor:'#033263' }}>
     ...
     <div style={{ marginTop:"150px" }}>
       ...
       
       {
        surah.map((e,i)=>{
           return(
               <div key={i} style={{ cursor:"pointer" }} className=" surat shadow-sm p-3 mb-5 bg-body rounded d-flex justify-content-around" onClick={ ()=>{ setSrc(e.id < 10 ? "00" + e.id : e.id >= 10 & e.id < 100 ? "0" + e.id.toString() : e.id.toString() )}}> 
                 <span> { e.name } </span>
                 <span> { e.id } </span>
                 <span> {e.arabic} </span> 
               </div>
